...
function __fish_complete_django -d "Completions for django" --argument-names cmd	complete -x -c $cmd
	...
	complete -c $cmd -n '__fish_seen_subcommand_from dumpdata' -s a -l all -d "Use Django's base manager to dump all models stored in the database, including those that would otherwise be filtered or modified by a custom manager."end